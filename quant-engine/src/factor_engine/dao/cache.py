...
class FactorCacheManager:
    ...
    def cache_technical_factor(self, stock_code: str, factor_name: str,
                              trade_date: date, factor_value: float) -> bool:
        """缓存技术因子数据"""
        try:
            key = self._build_key(
    ...
    def get_technical_factor(self, stock_code: str, factor_name: str,
                           trade_date: date) -> Any:
        """获取技术因子缓存数据"""
        try:
            key = self._build_key(
    ...
    def cache_technical_factors_batch(self, stock_code: str, trade_date: date,
    ...
    def get_technical_factors_batch(self, stock_code: str,
                                  trade_date: date) -> Any:
        """批量获取技术因子缓存数据"""
        try:
            key = self._build_key(
    ...
    def cache_fundamental_factor(self, stock_code: str, factor_name: str,
                               report_period: str, factor_value: float) -> bool:
        """缓存基本面因子数据"""
        try:
            key = self._build_key(
                self.key_prefix['fundamental'],
                stock_code,
                factor_name,
                report_period
            )
    ...
    def get_fundamental_factor(self, stock_code: str, factor_name: str,
                             report_period: str) -> Any:
        """获取基本面因子缓存数据"""
        try:
            key = self._build_key(
                self.key_prefix['fundamental'],
    ...
    def cache_market_factor(self, stock_code: str, factor_name: str,
                          trade_date: date, factor_value: float) -> bool:
        """缓存市场因子数据"""
        try:
            key = self._build_key(
    ...
    def get_market_factor(self, stock_code: str, factor_name: str,
                        trade_date: date) -> Any:
        """获取市场因子缓存数据"""
        try:
            key = self._build_key(
    ...
    def cache_sentiment_factor(self, stock_code: str, calculation_date: date,
                             factor_value: float, news_count: int) -> bool:
        """缓存新闻情绪因子数据"""
        try:
            key = self._build_key(
    ...
    def get_sentiment_factor(self, stock_code: str,
                           calculation_date: date) -> Any:
        """获取新闻情绪因子缓存数据"""
        try:
            key = self._build_key(
                self.key_prefix['news_sentiment'],
    ...
    def cache_calculation_result(self, task_id: str, result: dict) -> bool:
        ...
        try:
            ...
            return self.redis_client.setex(
                key,
                self.ttl_config['calculation_result'],
                serialized_data
            )
        except Exception as e:
            print(f"缓存计算结果失败: {e}")
...
def create_cache_manager(redis_host: str = 'localhost',
                        redis_port: int = 6379,
                        redis_db: int = 0,
                        redis_password: str | None = None) -> FactorCacheManager:
    """创建缓存管理器实例"""
    redis_client = Redis(
        host=redis_host,
        port=redis_port,
